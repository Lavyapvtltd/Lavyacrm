...
const TopBar = () => {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			...
			var hours = hours % 12;
			var time =
				('0' + hours).slice(-2) + ':' + ('0' + d.getMinutes()).slice(-2) + ampm;
						...
						<Col
							...
							<Dropdown className='topbar-head-dropdown topbar-tag-dropdown justify-content-end'>
								<Dropdown.Toggle
									...
									className='btn btn-icon btn-topbar rounded-circle text-white-50 fs-13 bg-transparent border-0 arrow-none dropdown-toggle btn btn-primary arrow-none'>
									<Image
